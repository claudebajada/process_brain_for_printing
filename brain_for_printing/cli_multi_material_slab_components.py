...
and a reverse transform to native space before meshing.
Includes an experimental feature to flatten mesh caps by snapping vertices.
"""
...
# --- Local Imports ---
# Ensure these point to the correct modules in your project structure
...
try:
    ...
    _L_HELPERS.warning("fslpy library not found. FLIRT matrix to world-world affine conversion will be limited "
                     "or require manual implementation of the transformation chain. "
                     "It is STRONGLY recommended to install fslpy for accurate processing.")
    pass
...
def get_flirt_world_to_world_affine(
    ...
    if not FSLPY_AVAILABLE:
        local_logger.error(
            ...
            f"FLIRT matrix {fsl_flirt_mat_path.name} to a world-to-world affine. Please install fslpy."
        )
        return None
    try:
...
def get_ideal_cap_coords_world_acpc(
    ...
    try:
        ...
        if ideal_coord_min_world > ideal_coord_max_world: # Ensure min is always less than max
            ideal_coord_min_world, ideal_coord_max_world = ideal_coord_max_world, ideal_coord_min_world
        # This log will now be inside the main loop for better context
        # logger.info(f"Ideal AC-PC world cap coords (for axis {slice_axis_idx_acpc}): "
        #             f"MinPlaneCoord={ideal_coord_min_world:.3f}, MaxPlaneCoord={ideal_coord_max_world:.3f}")
...
def flatten_mesh_caps_acpc(
    ...
    if mesh.is_empty:
        ...
        return mesh # Return original empty mesh

    original_vertices_native = mesh.vertices.copy() # Keep a copy of original vertices

    try:
        mesh_acpc = mesh.copy()
        mesh_acpc.apply_transform(native_to_acpc_world_affine)
        
        # This specific debug log for bounds will be in main_wf for better context
        # logger.debug(f"Mesh transformed to AC-PC space for cap flattening. Bounds: {mesh_acpc.bounds}")

        acpc_vertices = mesh_acpc.vertices.copy() # Work on a copy of AC-PC vertices
        modified_vertices_acpc = acpc_vertices.copy() # This will store changes

        target_coord_idx = slice_axis_idx_acpc # e.g., 2 for Z if axial

        # Min Cap
        min_cap_mask = np.abs(acpc_vertices[:, target_coord_idx] - ideal_coord_min_acpc) < tolerance_mm
        ...
        # Max Cap
        max_cap_mask = np.abs(acpc_vertices[:, target_coord_idx] - ideal_coord_max_acpc) < tolerance_mm
        ...
        if num_min_verts == 0 and num_max_verts == 0:
            logger.info("No vertices were snapped for either cap. Returning original mesh.")
            return mesh # Return original mesh if no changes

        # Create new mesh with modified AC-PC vertices, then transform back to native
        ...
        logger.debug("Processing flattened mesh (removing degenerate faces, fixing normals)...")
        flattened_mesh_native.remove_degenerate_faces()
        if flattened_mesh_native.is_empty: # Check after removing degenerate faces
             logger.error("Mesh became empty after removing degenerate faces post-flattening. Reverting to original.")
             return trimesh.Trimesh(vertices=original_vertices_native, faces=mesh.faces, process=True) # Return a processed version of original

        flattened_mesh_native.fix_normals(multibody=True) # multibody=True is safer
        
        # Perform final processing
        flattened_mesh_native.process(validate=True) # Validate after all changes
        if flattened_mesh_native.is_empty:
            logger.error("Mesh became empty after final processing post-flattening. Reverting to original.")
            return trimesh.Trimesh(vertices=original_vertices_native, faces=mesh.faces, process=True) # Return a processed version of original
        
        logger.info("Successfully flattened mesh caps and processed.")
        return flattened_mesh_native

    except Exception as e:
        logger.error(f"Error during mesh cap flattening for input mesh: {e}", exc_info=True)
        # Return a processed version of the original mesh on error to be safe
        return trimesh.Trimesh(vertices=original_vertices_native, faces=mesh.faces, process=True)

def acpc_align_t1w(
    input_t1w_path: Path,
    output_acpc_t1w_path: Path,
    output_transform_mat_path: Path, # This will be native-space-of-input-to-flirt -> mni_template_for_acpc
    mni_template_path: Path,
    temp_dir_path: Path, # For robustfov output
    logger: logging.Logger,
    verbose: bool = False
    ...
    # Use a more specific name for robustfov output to avoid clashes if input_t1w_path is already in temp_dir_path
    # (e.g., if it was already a robustfov output from a previous step/run)
    robustfov_out_path = temp_dir_path / f"{input_t1w_path.name.replace('.nii.gz','').replace('.nii','')}_robustfov.nii.gz"
    robustfov_cmd = ["robustfov", "-i", str(input_t1w_path), "-r", str(robustfov_out_path)]
    t1w_to_flirt_actual: Path = input_t1w_path # Default to original if robustfov fails

    try:
        logger.debug(f"Running robustfov command: {' '.join(robustfov_cmd)}")
        # Removed check=False, run_cmd handles return codes via its own 'check_return_code'
        ...
        if robustfov_out_path.exists() and robustfov_out_path.stat().st_size > 0:
            ...
            # t1w_to_flirt_actual remains input_t1w_path
    ...
    # The FSL .mat file name should be based on the *actual input to FLIRT*
    # This output_transform_mat_path is passed in, so its name should be correct based on caller's logic.
...
def resample_volume_to_native_space(
    ...
    original_native_t1w_ref_path: Path, # Defines the output grid (FoV, voxel size, sform)
    acpc_to_native_t1w_mat_path: Path,  # FSL .mat: transforms coords from AC-PC space to native space
    logger: logging.Logger,
    ...
    cmd = [
        "flirt",
        "-in", str(input_acpc_slab_volume_path),    # Input is in AC-PC space
        "-ref", str(original_native_t1w_ref_path), # Defines output grid in native space
        "-applyxfm",
        "-init", str(acpc_to_native_t1w_mat_path),  # Matrix from AC-PC space to native space
    ...
    try:
        ...
        if output_native_slab_volume_path.exists() and output_native_slab_volume_path.stat().st_size > 0:
            # Verify the output affine matches the reference T1w's affine
...
def get_volume_bounding_box_voxel_coords(volume_path: Path, logger: logging.Logger) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    try:
        ...
        data = img.get_fdata().astype(bool) # Ensure boolean for np.any and np.argwhere
        if not np.any(data):
...
def vol_to_mesh(volume_path: Path, output_mesh_path: Path, no_smooth: bool, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        if data_check.dtype == np.uint8 or data_check.dtype == bool: # For binary masks
            if data_check.size > 0 and np.sum(data_check) < 10: # Allow a few voxels for tiny structures
                is_effectively_empty = True
        elif data_check.size > 0 and np.max(np.abs(data_check)) < 0.01: # Heuristic for float PV maps, allow small values
             is_effectively_empty = True
        
        if is_effectively_empty:
            logger.info(f"Volume {volume_path.name} appears effectively empty (sum/max criteria). Skipping mesh generation.")
            return False # Indicates mesh not generated because volume empty
    except Exception as e:
    ...
    try:
        # Attempt to convert volume to GIFTI surface
        volume_to_gifti(str(volume_path), str(temp_gii_path), level=0.5) # Ensure volume_to_gifti is robust
        if temp_gii_path.exists() and temp_gii_path.stat().st_size > 0:
            mesh = gifti_to_trimesh(str(temp_gii_path)) # Ensure gifti_to_trimesh is robust
        else:
    ...
    if not isinstance(mesh, trimesh.Trimesh) or (hasattr(mesh, 'is_empty') and mesh.is_empty):
        ...
        return False # Mesh generation failed or resulted in an empty mesh
    
    # Process the mesh (validate, fill holes, smooth, fix normals)
    try:
        ...
        if not isinstance(mesh, trimesh.Trimesh) or mesh.is_empty:
             ...
             return False # Cannot proceed if mesh is compromised
        ...
        if not no_smooth:
            ...
            if mesh.vertices.shape[0] > 0 and mesh.faces.shape[0] > 0: # Check if mesh has content
...
# _build_parser remains the same
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--flatten_caps_tolerance_mm", type=float, default=0.5,
                        help="EXPERIMENTAL: If set, attempts to flatten mesh caps by snapping vertices. "
                             "Value is the tolerance in mm for selecting cap vertices in AC-PC space. "
                             "E.g., 0.5 or 1.0. Use with caution. Requires fslpy for best results.")
    return parser


def main_wf(args: argparse.Namespace, L: logging.Logger, work_dir: Path, stl_output_base_dir: Path, runlog: Dict[str, Any]):
    # ... (initial setup and logging from previous versions) ...
    ...
    mni_template_for_acpc = Path(args.mni_template) # Already resolved in main()

    # Determine actual source for FLIRT (robustfov output or original T1w)
    robustfov_out_path = acpc_align_dir / f"{original_t1w_path_for_ref.name.replace('.nii.gz','').replace('.nii','')}_croppedfov.nii.gz"
    flirt_src_for_acpc_alignment: Path = original_t1w_path_for_ref # Default
    if run_cmd(["robustfov", "-i", str(original_t1w_path_for_ref), "-r", str(robustfov_out_path)], verbose=args.verbose) == 0 and \
       robustfov_out_path.exists() and robustfov_out_path.stat().st_size > 0:
        flirt_src_for_acpc_alignment = robustfov_out_path
        L.info(f"robustfov completed, using {flirt_src_for_acpc_alignment.name} for FLIRT to AC-PC.")
    else:
        L.warning(f"robustfov failed or output empty. Using original T1w '{original_t1w_path_for_ref.name}' for FLIRT to AC-PC.")

    t1w_to_acpc_fsl_mat_path = acpc_align_dir / f"{flirt_src_for_acpc_alignment.name.replace('.nii.gz','').replace('.nii','')}_to_acpc.mat"
    acpc_success, _ = acpc_align_t1w( # Second return value (actual_flirt_src_path) is already flirt_src_for_acpc_alignment
        original_t1w_path_for_ref, # This is the base T1w, acpc_align_t1w handles robustfov internally now
        t1w_acpc_aligned_path,
        t1w_to_acpc_fsl_mat_path, # Pass the correctly named .mat path
        mni_template_for_acpc,
        acpc_align_dir, L, args.verbose
    )
    if not acpc_success: L.error("AC-PC alignment step failed."); return False
    runlog["steps"].append(f"AC-PC T1w: {t1w_acpc_aligned_path.name}, Native-to-ACPC FSL XFM: {t1w_to_acpc_fsl_mat_path.name}")

    acpc_to_native_fsl_mat_path = acpc_align_dir / f"{flirt_src_for_acpc_alignment.name.replace('.nii.gz','').replace('.nii','')}_acpc_to_native.mat"
    ...
    if FSLPY_AVAILABLE:
        ...
        # native_to_acpc_world_affine: Transforms from `flirt_src_for_acpc_alignment` world to `mni_template_for_acpc` world
        native_to_acpc_world_affine = get_flirt_world_to_world_affine(
            fsl_flirt_mat_path=t1w_to_acpc_fsl_mat_path,
            src_image_path=flirt_src_for_acpc_alignment, # This is the actual source used in FLIRT
        ...
        if native_to_acpc_world_affine is not None:
            ...
            try:
                ...
                L.error(f"Failed to invert native-to-ACPC world affine: {e_inv}. "
                        "ACPC-to-Native world affine will be unavailable.", exc_info=True)
                acpc_to_native_world_affine = None
        else:
            L.error("Failed to derive native-to-ACPC world affine using fslpy.")
            acpc_to_native_world_affine = None # Ensure this is also None
    else: # FSLPY_AVAILABLE is False
        L.warning("fslpy is not available. World-to-world affine transformations cannot be reliably computed. "
                  "Step 4 (voxelization) will proceed without explicit mesh transformation to AC-PC space, "
                  "which is LIKELY to cause misalignment. Cap flattening (Step 9) will be disabled.")
        runlog["warnings"].append("fslpy not available. Critical affine transforms skipped. Expect misalignment.")
    
    # If cap flattening was requested but affines failed, disable it now.
    if args.flatten_caps_tolerance_mm is not None and (native_to_acpc_world_affine is None or acpc_to_native_world_affine is None):
        L.warning("Disabling cap flattening as necessary world-to-world affines could not be computed.")
    ...
    # ... (This part remains largely the same as your previous version, ensuring parent_meshes_trimesh is populated)
    ...
    for item in white_other_for_complex_from_preset:
        if item == "cerebellum_wm": white_other_explicit_for_white_complex.add("cerebellum_wm")
            ...
            white_other_explicit_for_white_complex.add("cerebellum_wm") # Assuming cerebellum_wm if 'cerebellum' is in white_brain
    ...
    if is_vtk_available(): # Check if VTK is available before attempting to use it
        ...
        if fs_input_dir.is_dir():
            ...
            if run_5ttgen_hsvs_save_temp_bids(args.subject_id, str(fs_input_dir), str(five_tt_gen_dir), args.session, verbose=args.verbose):
                vtk_meshes = load_subcortical_and_ventricle_meshes(str(five_tt_gen_dir)) # This function loads VTK, make sure it's robust
    ...
    # temp_obj_export_dir was: parent_surf_gen_dir / "temp_full_surf_objs_acpc_aligned"
    # Let's make it inside full_vol_voxelized_dir to keep Step 4 outputs together
    ...
    for parent_name, trimesh_native_obj in valid_parent_meshes_trimesh.items():
        if trimesh_native_obj is None: continue # Should have been filtered by valid_parent_meshes_trimesh

        safe_name = parent_name.replace("_", "-").replace(" ", "-")
        # This OBJ will store vertices in AC-PC world space if transform is applied
        ...
        mesh_to_voxelize_acpc = trimesh_native_obj.copy() # Native space mesh
        ...
        if native_to_acpc_world_affine is not None:
            ...
            # debug_stl_path = temp_obj_export_dir_for_acpc_voxelization / f"{args.subject_id}_desc-{safe_name}_DEBUG_transformed_to_acpc.stl"
            # mesh_to_voxelize_acpc.export(str(debug_stl_path))
            # L.debug(f"Saved AC-PC transformed mesh for debugging: {debug_stl_path}")
        else:
            L.critical(f"CRITICAL: Native-to-ACPC world affine is MISSING for surface '{parent_name}'. "
                      f"Voxelization onto AC-PC template ('{master_hires_acpc_template_path.name}') will be INCORRECT (surface effectively still in native space).")
            runlog["warnings"].append(f"CRITICAL_VOXELIZATION_ERROR: {parent_name} voxelized without AC-PC transform.")
            # Decide if you want to continue or abort if this happens. For now, it proceeds but will be misaligned.
            # To be safer, one might 'continue' to skip this problematic surface:
            # L.error(f"Skipping voxelization of {parent_name} due to missing critical transform.")
            # continue
        ...
        if binarize_volume_file(full_pv_path, full_bin_path, args.pv_threshold, L):
            ...
            # DEBUG: Crucial check point for the image provided by the user
            L.info(f"Generated AC-PC binarized mask: {full_bin_path}. "
                   f"VISUALLY CHECK alignment with: {t1w_acpc_aligned_path} and {master_hires_acpc_template_path}")
        else: L.warning(f"Failed to binarize full PV for {parent_name}.")
        
        # Clean up individual AC-PC aligned OBJ if not keeping work dir
    ...
    # Clean up the temporary OBJ directory
    ...
    # ... (rest of script from previous version, assuming Step 5, 6, 7, 8, 9 are mostly okay IF Step 4 is correct) ...
    ...
    for struct_name, mask_path in full_acpc_binarized_masks.items():
        ...
        if bounds_vox:
            ...
            if overall_min_coords_vox is None: # First valid bounds
    ...
    cropped_acpc_grid_affine_for_caps: Optional[np.ndarray] = None # For ideal cap coords
    
    for struct_name, full_mask_path in full_acpc_binarized_masks.items():
        safe_name = struct_name.replace("_", "-").replace(" ", "-") # Redundant if already safe, but harmless
        ...
        if crop_nifti_volume_fslroi(full_mask_path, cropped_path, overall_min_coords_vox, overall_max_coords_vox, L, args.verbose):
            ...
            # Get affine from the *first successfully cropped* volume for cap calculations
            if cropped_acpc_grid_affine_for_caps is None: # and args.flatten_caps_tolerance_mm is not None: (this condition removed as it might be needed anyway)
                try:
    ...
    # Re-check if cap flattening should be disabled if the grid affine is missing
    ...
    # Voxel size on slicing axis from the master hires template
    ...
    current_slab_start_voxel_in_cropped = 0 # Start from voxel 0 of the CROPPED volume
    volumetric_slabs_acpc_struct_files: Dict[int, Dict[str, Path]] = {} # slab_idx -> {struct_name: path}
    slab_definitions_voxel_acpc: List[Dict[str, Any]] = [] # Store slab start/thickness in cropped voxels
    ...
    while current_slab_start_voxel_in_cropped < total_extent_cropped_vox:
        ...
        if actual_slab_thickness_this_iteration_vox <= 0: break # No more voxels left to form a slab
        ...
        for struct_name, cropped_vol_path in cropped_acpc_binarized_masks.items():
            safe_name = struct_name.replace("_", "-").replace(" ", "-") # Already safe, but consistent
    ...
    # --- Loop through each defined slab index (0 to num_slabs_generated - 1) ---
    for slab_idx in range(num_slabs_generated):
        ...
        def load_acpc_slab_nifti(struct_name_key: str) -> Optional[np.ndarray]:
            nonlocal example_acpc_slab_nifti_path_for_saving_template # Allow modification
            path_to_load = current_slab_acpc_component_paths.get(struct_name_key)
            if path_to_load and path_to_load.exists():
                if example_acpc_slab_nifti_path_for_saving_template is None: # Grab the first valid one
        ...
        # Pial Complex for this slab
        pial_keys = ["pial_L", "pial_R", "corpus_callosum", "cerebellum", "brainstem"] # From preset
        ...
        # White Complex for this slab
        white_keys = ["white_L", "white_R", "corpus_callosum", "cerebellum_wm", "brainstem"] # From preset
        ...
        # Ventricles for this slab
        vent_keys = [k for k in valid_parent_meshes_trimesh if k.startswith("ventricle-")] # Dynamic from 5ttgen
        ...
        # SGM for this slab
        sgm_keys = [k for k in valid_parent_meshes_trimesh if k.startswith("subcortical-")] # Dynamic from 5ttgen
        ...
        if example_acpc_slab_nifti_path_for_saving_template is None: # Should have been set by load_acpc_slab_nifti
        ...
        material_acpc_slab_volumes_to_resample: Dict[str, Path] = {} # Store paths of successfully saved AC-PC material volumes

        # OuterCSF = BrainMask - PialComplex
        ...
        # GreyMatter = PialComplex - WhiteComplex
        ...
        # Ventricles (already combined, ensure it's not None)
        ...
        # SubcorticalGrey (already combined, ensure it's not None)
        ...
        # WhiteMatter = WhiteComplex - Ventricles - SubcorticalGrey
        ...
        # --- Step 8: Resample AC-PC Material Slabs to Native T1w Space ---
        ...
        for mat_name, acpc_vol_path in material_acpc_slab_volumes_to_resample.items():
            ...
            if resample_volume_to_native_space(
                acpc_vol_path, native_vol_path, original_t1w_path_for_ref, # This must be the original T1w
                acpc_to_native_fsl_mat_path, # FSL .mat from AC-PC to Native
        ...
        if can_flatten_this_slab:
            ideal_cap_coords_world_for_slab = get_ideal_cap_coords_world_acpc(
                ...
                cropped_acpc_grid_affine=cropped_acpc_grid_affine_for_caps, # type: ignore
                logger=L
            )
            if ideal_cap_coords_world_for_slab:
                ...
                can_flatten_this_slab = False # Disable for this slab
        elif args.flatten_caps_tolerance_mm is not None: # It was requested but prerequisites failed earlier
            L.warning(f"Slab {slab_idx}: Prerequisites for cap flattening not met. Skipping for this slab's materials.")
        ...
        for mat_name in material_vol_names_for_mesh:
            ...
            if native_vol_path_for_mesh and native_vol_path_for_mesh.exists():
                ...
                if mesh_generated_successfully and can_flatten_this_slab and ideal_cap_coords_world_for_slab:
                    L.info(f"Attempting to flatten caps for {stl_path.name} (Slab {slab_idx}, Mat {mat_name})...")
                    # --- DEBUGGING BLOCK START ---
                    L.debug(f"--- Debug Info for Cap Flattening: Slab {slab_idx}, Material {mat_name} ---")
                    L.debug(f"  Native mesh STL path: {stl_path}")
                    L.debug(f"  Native-to-ACPC World Affine (native_to_acpc_world_affine):\n{native_to_acpc_world_affine}") # type: ignore
                    L.debug(f"  Calculated Ideal AC-PC World Cap Coords (axis {slice_axis_idx}): "
                            f"Min={ideal_cap_coords_world_for_slab[0]:.4f}, Max={ideal_cap_coords_world_for_slab[1]:.4f}")
                    try:
                        loaded_mesh_for_debug = trimesh.load_mesh(str(stl_path))
                        if not loaded_mesh_for_debug.is_empty:
                            mesh_in_acpc_for_debug = loaded_mesh_for_debug.copy()
                            mesh_in_acpc_for_debug.apply_transform(native_to_acpc_world_affine) # type: ignore
                            L.debug(f"  Mesh '{stl_path.name}' Bounds when transformed to AC-PC space:\n{mesh_in_acpc_for_debug.bounds}")
                            if mesh_in_acpc_for_debug.vertices.shape[0] > 0:
                                L.debug(f"  Example transformed vertex coords along slice_axis_idx={slice_axis_idx} (first 5): "
                                        f"{mesh_in_acpc_for_debug.vertices[:5, slice_axis_idx]}")
                                L.debug(f"  Min/Max of transformed vertex coords along slice_axis_idx={slice_axis_idx}: "
                                        f"Min={mesh_in_acpc_for_debug.vertices[:, slice_axis_idx].min():.4f}, "
                                        f"Max={mesh_in_acpc_for_debug.vertices[:, slice_axis_idx].max():.4f}")
                        else: L.debug(f"  Could not load mesh {stl_path.name} for debug bounds check (it's empty after loading).")
                    except Exception as e_debug_load: L.debug(f"  Error loading mesh {stl_path.name} for debug bounds check: {e_debug_load}")
                    L.debug(f"--- End Debug Info for Cap Flattening ---")
                    # --- DEBUGGING BLOCK END ---
                    try:
                        loaded_mesh_for_flattening = trimesh.load_mesh(str(stl_path)) # Reload fresh
                        if not loaded_mesh_for_flattening.is_empty:
                            ideal_coord_min, ideal_coord_max = ideal_cap_coords_world_for_slab
                            flattened_mesh = flatten_mesh_caps_acpc(
                                mesh=loaded_mesh_for_flattening,
                                native_to_acpc_world_affine=native_to_acpc_world_affine, # type: ignore
                                acpc_to_native_world_affine=acpc_to_native_world_affine, # type: ignore
                                ideal_coord_min_acpc=ideal_coord_min,
                                ideal_coord_max_acpc=ideal_coord_max,
                                slice_axis_idx_acpc=slice_axis_idx,
                                tolerance_mm=args.flatten_caps_tolerance_mm, # type: ignore
                                logger=L
                            )
                            if flattened_mesh and not flattened_mesh.is_empty:
                                # Check if vertices actually changed before declaring success
                                if not np.array_equal(flattened_mesh.vertices, loaded_mesh_for_flattening.vertices):
                                    L.info(f"Caps were modified for {stl_path.name}. Exporting flattened version.")
                                    flattened_mesh.export(str(stl_path))
                                    runlog["steps"].append(f"Caps flattened for {mat_name} slab {slab_idx}")
                                else:
                                    L.info(f"Flattening did not change vertices for {stl_path.name} (likely no vertices in tolerance or reverted). Original mesh retained.")
                            elif flattened_mesh is None or flattened_mesh.is_empty: # Explicitly check for None too
                                L.warning(f"Cap flattening returned None or an empty mesh for {stl_path.name}. Original mesh retained.")
                        else:
                        ...
                        L.error(f"Error during cap flattening for {stl_path.name}: {e_flatten}. Original mesh retained.", exc_info=True)
                ...
                elif mesh_generated_successfully: # Mesh was made, but now it's gone (likely due to cap flattening issue)
                     L.warning(f"Mesh {stl_path.name} was generated by vol_to_mesh but is now missing or empty after cap flattening attempt.")
                     runlog["warnings"].append(f"Mesh {stl_path.name} missing/empty after cap flattening attempt.")
            else:
...
def main():
    ...
    _L_HELPERS.setLevel(L_main.getEffectiveLevel()) # Ensure helper logger also respects verbosity
    ...
    # Resolve and check MNI template path
    ...
    if args.work_dir:
        ...
        try:
            ...
            if args.no_clean : L_main.info("--no_clean is active. Retaining work directory.") # Simplified message
        except Exception as e:
        ...
        temp_base_dir = Path(args.output_dir).resolve() # Ensure it's absolute
        ...
        try:
            with temp_dir_context as temp_d_str:
                ...
                if args.no_clean and temp_work_dir_path.exists(): # Check if it exists before logging retention
    ...
    # Finalize runlog (ensure all paths are strings)
    if "args" in runlog:
